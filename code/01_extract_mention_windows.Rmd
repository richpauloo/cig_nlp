...
read_pdfs_pages <- function(p){
  length(pdf_text(paste0(fp, "all_papers/", p)))
}
...
#       {
#         print(paste(i, "Now reading:", p[i]))
#         length(pdf_text(paste0(fp, "all_papers/", p[i])))
#       },
#       error = function(cond){
#         message(paste(i, "Error reading:", p[i]))
#         message(cond)
...
# fix errors
#n_pages[c(151,314,395)] <- c(11,34,47)
...
df1 <- mutate(df1, software_name = case_when(software_name %in% c("axisem", "AXISEM") ~ "AxiSEM",
                                             ...
                                             software_name == "Generic Mapping Tool" ~ "GMT",
                                             TRUE ~ software_name))
...
ggsave(p1, filename = "C:/Users/rpauloo/Documents/GitHub/cig_nlp/rich_plots/cig_total_and_unique_mentions_trunc.pdf", device = cairo_pdf, width = 11, height = 8)
...
for(i in 1:length(p_list)){
  p_list[[i]] <- final %>% 
    ...
    ggplot(aes(x = factor(paper))) +
    geom_point(aes(y = token_position), color = "black", size = 2) +
    geom_segment(aes(y = error_low_p, yend = error_high_p, xend = factor(paper)), color = "red") +
    coord_flip() + 
...
pdf("C:/Users/rpauloo/Documents/GitHub/cig_nlp/rich_plots/software_mentions.pdf", width = 11 , height = 8)
...
ggsave(co_plot, filename = "C:/Users/rpauloo/Documents/GitHub/cig_nlp/rich_plots/co_occurrence_plot.pdf", device = cairo_pdf, height = 5, width = 15)
```
