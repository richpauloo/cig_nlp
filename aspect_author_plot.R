...
E(aspect.net)$x
E(aspect.net)$width1 = E(aspect.net)$x #some bug when edge named x?

author.plot = ggraph(aspect.net, layout = "kk") + 
  geom_edge_link(aes(width = width1), alpha = 1, color = "gray") +
...
ggplot(reshape2::melt(centrality_measures, idvars = 'name', value.name = "rank") %>% group_by(variable)) +
  geom_line(aes(x = fct_reorder(name, rank), y = rank, group = name)) +
  geom_point(aes(x = fct_reorder(name, rank), y = rank, group = name, color = variable)) +
